...
\date{Version 6.0 (corrected) --- July 2026}
% ============================
\begin{document}
\maketitle
...
We are explicit about what remains open: (i) the covariant action for the pillar; (ii) the spatial lensing offset of the Bullet Cluster, which we show is \emph{not} produced by the memory mechanism --- the same wall faced by all modified-gravity approaches --- and remains unsolved; and (iii) the CMB acoustic-peak amplitudes, which require a dedicated non-local Boltzmann code. The CMB peak \emph{geometry} is shown compatible with FSG.
\end{abstract}
\newpage
\tableofcontents
\newpage
