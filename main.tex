...
% ============================
\begin{abstract}
We propose a modified theory of gravity based on a Fractal and Non-Local
Spacetime Geometry (FSG), obtained from an action containing the non-local
scalar operator $X = \Box^{-1}R$. The effective action
\[
...
Cosmologically, the model predicts a phantom-like equation of state ($w_0 < -1$), measurable deviations
$\Delta H/H\simeq 2$--$6\%$ for $z<2$, and early structure formation consistent with JWST. The theory is fully falsifiable via precision measurements of Euclid and Roman space telescopes.
\end{abstract}
...
% ============================================================
\section{Physical Structure of the Non-Local Terms}
% ============================================================

The non-local kernel produces ``memory terms'' involving:
\begin{itemize}
    \item gradients $\nabla_\mu X\,\nabla_\nu X$,
    \item cross-terms $\nabla_\mu X\,\nabla_\nu f'(X)$,
    \item higher-order inverse operators.
\end{itemize}

The key physical consequence is the \emph{non-local memory} of spacetime:
\begin{equation}
X(x) = \int d^4x'\,
G_{\rm ret}(x,x')\, R(x'),
\label{eq:memory}
\end{equation}
meaning the present geometry depends on the full past curvature.

This explains:
\begin{itemize}
    \item apparent dark-matter-like offsets in merging clusters (e.g.~Bullet Cluster),
    \item early formation of large-scale structure,
    \item the smooth MOND--GR transition without extra vector fields.
\end{itemize}
...
\begin{align}
G(k) &\sim \frac{1}{k^2 (1 + \mathcal{O}(k))} \quad \text{(UV regime, } k \to \infty\text{)} \\
G(k) &\sim \frac{1}{k^2 \cdot k} = \frac{1}{k^3} \quad \text{(IR regime, } k \to 0\text{)}.
\end{align}
...
\begin{equation}
\mathbf{G(k) \xrightarrow{IR} \frac{1}{k^3}.}
\label{eq:Gk3}
\end{equation}
...
\end{equation}
This follows directly from the scaling $f(X_k) \sim k$ derived in the previous section.
...
Thus FSG produces flat rotation curves automatically.


% ---------------- FIGURE ----------------
\begin{figure}[h!]
\centering
\includegraphics[width=0.70\textwidth]{fig_rotation_curves.pdf}
\caption{
Rotation curve comparison for a typical spiral galaxy. Newtonian prediction
falls as $1/\sqrt{r}$, while FSG remains flat and matches observations.}
\label{fig:rotation}
\end{figure}
...
\subsection{The Mechanism (Heuristic)}
In FSG, the infrared dimensional reduction ($d_S \to 2$) modifies the effective Green's function. We conjecture that in the early universe, this manifests as a scale-dependent enhancement of the gravitational coupling $G_{\rm eff}(k)$.
The modified perturbation equation would take the form:
\begin{equation}
...
\newpage
% ============================================================
\section{Conceptual Limitations of FSG}
% ============================================================

Despite its successes, the theory has current limitations that must be addressed in future work:
...
\subsection{Galactic-to-Cosmological Matching}
Galaxies are quasi-static systems, while cosmology is evolving. 
The transition is governed by the infrared scale $L$, but a deep derivation of the FSG kernel in curved, time-dependent backgrounds is required to fully understand how the static $1/k^3$ propagator emerges from the cosmological evolution.

\subsection{Absence of a Boltzmann Code}
To compare FSG with CMB data quantitatively, a full Boltzmann code (modified CLASS/CAMB) must be implemented. This code needs to include:
\begin{itemize}
    \item the scale-dependent fractal propagator $G(k)$,
    \item the evolution of non-local auxiliary fields $U$ and $\xi$,
    \item the time-dependent transition of the spectral dimension.
\end{itemize}
This is technically feasible but not yet available.

\subsection{Microscopic Interpretation of IR Fractality}
Dimensional reduction is known in UV quantum gravity (asymptotic safety, CDT), but IR reduction is a new concept.
A fundamental interpretation (holographic or quantum geometric) of why the universe loses degrees of freedom at large scales remains to be constructed.

% ============================================================
\section{Observational Limitations}
% ============================================================

\subsection{Uncertainties in \texorpdfstring{$H(z)$}{H(z)} at Intermediate Redshift}
Current data (Pantheon+, BAO, cosmic chronometers) have too large uncertainties
to distinguish a 2--3\% deviation.  
Euclid will resolve this.

\subsection{Baryonic Mass Uncertainties}
SPARC includes galaxies with uncertain gas masses.  
FSG fits remain better than dark matter models, but uncertainties lead to small
residual discrepancies in extreme cases.

\subsection{Galaxy Clusters and Scale Dependence}
Galaxy clusters pose a specific challenge. Preliminary phenomenology suggests that clusters might require a stronger IR enhancement ($\epsilon \approx 0.25$) compared to individual galaxies ($\epsilon \approx 0.1-0.15$).
If confirmed, this would imply that the dimensional deficit $\epsilon$ is not a universal constant but depends on the depth of the potential well or the curvature scale $R$. This **running of the fractal dimension** is a known feature in Quantum Gravity approaches (e.g., CDT) but implies that the current FSG action is a simplified effective description of a more complex scale-dependent geometry.
...
\subsection{Breakdown of the RAR}
If future high-resolution surveys find galaxies deviating from:
\[
g_{\rm obs} = \sqrt{g_{\rm bar}a_0}
\quad (g_{\rm bar}<a_0),
\]
then FSG is falsified.
...
\newpage
% ============================================================
\section{Simulation Code (Core Logic)}
% ============================================================
\label{app:code}
