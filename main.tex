...
A perturbative treatment of the non-local sector around the cosmological background shows that Solar System constraints are satisfied \emph{without any screening mechanism} ($\delta\gamma \sim 10^{-20}$), for the same structural reason as in the related RR non-local model. In the strong-field regime, Schwarzschild and Kerr are exact vacuum solutions of FSG (since $R=0$ switches off the non-local term), so black-hole observables --- shadows, ringdown, ISCO --- are identical to General Relativity; any deviation is confined beyond the MOND radius $r_M=\sqrt{GM/a_0}$, $8$--$11$ orders of magnitude outside the horizon. Cosmologically, the monotonic growth of the memory fields structurally implies a phantom-like equation of state ($w_0 < -1$); a first-order estimate brackets the deviation of $H(z)$ between $\sim 1\%$ and $\sim 13\%$ for $0.5<z<2$, testable by Euclid.
...
\subsection{The Honest Logical Structure}
The logical chain of FSG is therefore the following:
\begin{enumerate}
  \item the \emph{nonlinear, position-space} derivation of the modified Poisson equation from the quasi-static limit of the action (next section) --- this is the founding derivation of the MOND regime in FSG;
  \item the Fourier-space statement \eqref{eq:Gk3} as the exact translation of that result;
  \item the spectral-dimension flow $d_S \to 2$ as the \emph{motivation} for the fractional exponent $-1/2$ in the action, not as a derivation of $\eta$. (Indeed, a naive application of anisotropic-scaling dictionaries, e.g.\ Ho\v{r}ava-type, would relate $d_S = 2$ to $k^{-6}$, not $k^{-3}$; the spectral dimension does not fix the static propagator by itself.)
\end{enumerate}
% ---------------- FIGURE ----------------
\begin{figure}[h!]
\centering
...
\end{figure}
% ============================================================
\section{The Founding Derivation: Modified Poisson Equation and the MOND Limit}
% ============================================================
...
\subsection{The Quasi-Static Limit and the Effective AQUAL Lagrangian}
We consider a perturbed metric $ds^2 = -(1+2\Phi)dt^2 + (1-2\Psi)dx^i dx_i$ in the quasi-static limit. For such configurations the Ricci scalar is dominated by $R \simeq 2\nabla^2\Phi/c^2$, and the inverse d'Alembertian reduces to the inverse Laplacian, so that the non-local field is finite and \emph{dimensionless}:
\begin{equation}
X = \Box^{-1}R \;\simeq\; \frac{2\Phi}{c^2} + X_{\rm cosmo},
\end{equation}
where $X_{\rm cosmo} = \mathcal{O}(10)$ is the background value accumulated by cosmic history (Section~\ref{sec:memory}). In the galactic outskirts the local contribution competes with the gradient energy of the potential, and the quasi-static reduction of the term $R\,f(X)$ produces an effective Lagrangian for $\Phi$ of the Bekenstein--Milgrom (AQUAL) type:
\begin{equation}
\mathcal{L}_{\rm eff} = -\frac{a_0^2}{8\pi G}\, F\!\left(\frac{|\nabla\Phi|^2}{a_0^2}\right) + \rho\,\Phi .
\label{eq:aqual}
\end{equation}
The role of the fractional exponent is the following \emph{scaling selection}: under the deep-IR scale invariance $(t,\vec{x}) \to (\lambda t, \lambda\vec{x})$ characteristic of the MOND regime \cite{Milgrom2014}, the unique power-law branch of $F$ compatible with the $X^{-1/2}$ structure of the action is
\begin{equation}
F(y^2) \longrightarrow \tfrac{2}{3}\,y^{3} \qquad (y = |\nabla\Phi|/a_0 \ll 1),
\end{equation}
i.e.\ the cubic deep-MOND branch, while $F(y^2) \to y^2$ (Newtonian) for $y \gg 1$. We emphasize the epistemic status of each step: the passage from \eqref{eq:aqual} to the results below is \emph{exact}; the identification of the exponent $-1/2$ with the cubic branch is a scaling correspondence whose $\mathcal{O}(1)$ coefficient requires the complete covariant reduction of $\Delta G_{\mu\nu}$, which we leave as an explicit open task (Section on future work).
\subsection{The Modified Poisson Equation}
Variation of \eqref{eq:aqual} yields the nonlinear \textbf{Modified Poisson Equation}
\begin{equation}
\nabla\cdot\!\left[\,\mu\!\left(\frac{|\nabla\Phi|}{a_0}\right)\nabla\Phi\,\right] = 4\pi G\rho,
\qquad \mu(y) = F'(y^2)\,,
\label{eq:modified_poisson}
\end{equation}
with $\mu(y) \to y$ in the deep-IR branch selected above. For a point source $M$, Gauss's theorem gives exactly
\begin{equation}
\frac{|\nabla\Phi|}{a_0}\,|\nabla\Phi| = \frac{GM}{r^2}
\;\;\Longrightarrow\;\;
|\nabla\Phi| = \frac{\sqrt{GM a_0}}{r}
\;\;\Longrightarrow\;\;
\Phi(r) = \sqrt{GM a_0}\,\ln\frac{r}{r_0}.
\label{eq:milgrom_exact}
\end{equation}
This is Milgrom's deep-MOND law, obtained here in position space with no reference to a linearized propagator. In Fourier language, Eq.~\eqref{eq:milgrom_exact} corresponds to the effective scaling $\Phi(k) \sim k^{-3}\rho(k)$, i.e.\ $\eta = 1$: the form factor $\Gamma(k) \sim (kL)^{\eta}$ used in earlier versions of this work is thus the \emph{Fourier-space name} of the nonlinear result \eqref{eq:milgrom_exact}, not an independent derivation.
Thus, the logarithmic potential is not an ansatz but the exact solution of the Modified Poisson Equation derived from the quasi-static limit of the FSG action.
\newpage
% ============================================================
...
\begin{itemize}
    ...
    \item Exact recovery of GR in the strong field: Schwarzschild/Kerr are vacuum solutions, so black-hole observables are unmodified.
...
% ============================================================
\subsection{Covariant Back-Reaction (Priority 1)}
Several v6 results are computed at zeroth or first order on a $\Lambda$CDM background: the memory-field values $(U_0, V_0)$ entering $a_0$, and the bracket on $w(z)$ and $H(z)$. The complete covariant integration of the FSG Friedmann equations --- including the energy--momentum tensor of the non-local sector --- is required to (i) close the $5\%$ residual on $a_0$, (ii) collapse the $[-1.7, -1.1]$ bracket on $w_0$ to a definite value, and (iii) derive from first principles the combination $|V|^{3/2}/|U|$ and the $\mathcal{O}(1)$ coefficient of the AQUAL $\mu$-function.
\subsection{Boltzmann Code Implementation (Priority 2)}
